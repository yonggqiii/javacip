class c19467546 {
    ...
    public boolean saveCart(Carrito cart, boolean completado, String date, String formPago) {
        ...
        try {
            ...
            insertHistorial = conexion.prepareStatement("INSERT INTO " + JavaCIPUnknownScope.nameBD + ".HistorialCarritos VALUES (?,?,?,?,?,?)");
            ...
            if (filasAfectadas != 1) {
                ...
                insertCarrito = conexion.prepareStatement("INSERT INTO " + JavaCIPUnknownScope.nameBD + ".Carritos VALUES (?,?,?,?,?)");
                Iterator<String> iteradorProductos = cart.getArticulos().keySet().iterator();
                while (iteradorProductos.hasNext()) {
                    String key = iteradorProductos.next();
                    Producto prod = JavaCIPUnknownScope.getProduct(key);
            ...
            JavaCIPUnknownScope.logger.log(Level.SEVERE, "Error añadiendo carrito al registro", ex);
            try {
                conexion.rollback();
            } catch (SQLException ex1) {
                JavaCIPUnknownScope.logger.log(Level.SEVERE, "Error haciendo rollback de la transacción para insertar carrito en el registro", ex1);
            }
        } finally {
            JavaCIPUnknownScope.cerrarConexionYStatement(conexion, insertCarrito, insertHistorial);
        }
        return exito;
    }
}
