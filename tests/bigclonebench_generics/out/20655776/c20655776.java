class c20655776 {
    ...
    public static void loginLocalhostr() throws IOException {
        ...
        Iterator<Cookie> it = JavaCIPUnknownScope.httpclient.getCookieStore().getCookies().iterator();
        Cookie escookie = null;
        while (it.hasNext()) {
            escookie = it.next();
            if (escookie.getName().contains("session")) {
                JavaCIPUnknownScope.sessioncookie = escookie.getName() + " = " + escookie.getValue();
