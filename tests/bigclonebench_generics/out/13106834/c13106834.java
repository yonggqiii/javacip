        ...
        for (String key : filenames) {
            ...
            if (key.equals("fw4ex.xml")) {
                try {
                    ...
                    FileWriter out = new FileWriter(new File(JavaCIPUnknownScope.project.getLocation() + "/" + JavaCIPUnknownScope.bundle.getString("Stem") + JavaCIPUnknownScope.STEM_FILE_EXETENSION));
                    int c;
                ...
                try {
                    FileReader in = new FileReader(new File(realPath));
                    FileWriter out = new FileWriter(new File(JavaCIPUnknownScope.project.getLocation() + "/" + key));
                    int c;
                    while ((c = in.read()) != -1) out.write(c);
                    in.close();
                    out.close();
