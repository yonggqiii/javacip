class c1039796 {
    ...
    public static void insert(Connection c, MLPApprox net, int azioneId, String descrizione, int[] indiciID, int output, Date from, Date to) throws SQLException {
        try {
            PreparedStatement ps = c.prepareStatement(JavaCIPUnknownScope.insertNet, PreparedStatement.RETURN_GENERATED_KEYS);
            ...
            if (key.next()) {
                ...
                for (int i = 0; i < indiciID.length; i++) {
                    PreparedStatement psIndex = c.prepareStatement(JavaCIPUnknownScope.insertNetIndex);
