class c13387011 {
    ...
    private void updateViewerContent(ScrollingGraphicalViewer viewer) {
        ...
        if (JavaCIPUnknownScope.withContent != null) {
            p.updateContentWith(JavaCIPUnknownScope.withContent);
        }
        BioPAXGraph newGraph = JavaCIPUnknownScope.main.getRootGraph().excise(p);
        newGraph.setAsRoot();
        viewer.setContents(newGraph);
        boolean layedout = newGraph.fetchLayout();
        if (!layedout) {
            new CoSELayoutAction(JavaCIPUnknownScope.main).run();
        }
        viewer.deselectAll();
