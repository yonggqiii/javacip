    ...
    public void performUpdates(List<PackageDescriptor> downloadList, ProgressListener progressListener) throws IOException, UpdateServiceException_Exception {
        ...
        try {
            for (PackageDescriptor desc : downloadList) {
                String urlString = JavaCIPUnknownScope.service.getDownloadURL(desc.getPackageId(), desc.getVersion(), desc.getPlatformName());
                ...
                if (desc.getPackageTypeName().equals("RAPIDMINER_PLUGIN")) {
                    ...
                    if (incremental) {
                        ...
                        try {
                            JavaCIPUnknownScope.updatePluginIncrementally(extension, JavaCIPUnknownScope.openStream(url, progressListener, minProgress, maxProgress), baseVersion, desc.getVersion());
                        } catch (IOException e) {
                    ...
                    if (!incremental) {
                        ...
                        JavaCIPUnknownScope.updatePlugin(extension, JavaCIPUnknownScope.openStream(url, progressListener, minProgress, maxProgress), desc.getVersion());
                    }
                    ...
                    JavaCIPUnknownScope.updateRapidMiner(JavaCIPUnknownScope.openStream(url, progressListener, minProgress, maxProgress), desc.getVersion());
                }
                i++;
