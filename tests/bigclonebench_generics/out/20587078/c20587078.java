class c20587078 {
    ...
    public static void main(String[] a) {
        ...
        new File(JavaCIPUnknownScope.outputDir).mkdirs();
        try {
            FileReader fis = new FileReader(JavaCIPUnknownScope.completeFileWithDirToCathFileList);
            ...
            for (String file : allFilesToBeCopied) {
                try {
                    FileChannel srcChannel = new FileInputStream(JavaCIPUnknownScope.CathDir + file).getChannel();
                    FileChannel dstChannel = new FileOutputStream(JavaCIPUnknownScope.outputDir + file).getChannel();
