class c9705209 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (mode.equals("join")) {
            ...
            JavaCIPUnknownScope.logger.info("putting file " + file);
            ...
            if (dht.put((short) appId, keyStr.getBytes(), value, ttl, secretStr.getBytes()) < 0) {
                JavaCIPUnknownScope.logger.info("There was an error while putting a key-value.");
                System.exit(0);
            }
            ...
            JavaCIPUnknownScope.logger.info("getting file " + file);
            ArrayList<byte[]> values = new ArrayList<byte[]>();
            if (dht.get((short) appId, keyStr.getBytes(), Integer.MAX_VALUE, values) < 0) {
                JavaCIPUnknownScope.logger.info("There was an error while getting a value.");
                System.exit(0);
            }
            ...
            JavaCIPUnknownScope.logger.info("removing <key,value> for key=" + keyStr);
            if (dht.remove((short) appId, keyStr.getBytes(), secretStr.getBytes()) < 0) {
                JavaCIPUnknownScope.logger.info("There was an error while removing a key.");
                System.exit(0);
            }
            System.out.println("Ok!");
            ...
            JavaCIPUnknownScope.logger.info("updating <key,value> for key=" + keyStr);
            if (dht.updateLease((short) appId, keyStr.getBytes(), ttl) < 0) {
                JavaCIPUnknownScope.logger.info("There was an error while updating data lease.");
                System.exit(0);
            }
            System.out.println("Ok!");
        }
