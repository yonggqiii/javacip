class c5620476 {
    ...
    private String[] verifyConnection(Socket clientConnection) throws Exception {
        ...
        String uri = JavaCIPUnknownScope.decodePercent(st.nextToken());
        if (!st.hasMoreTokens()) {
        ...
        while (in.ready() && (line = in.readLine()) != null) {
            ...
            JavaCIPUnknownScope.logger.debug("HTTP Request Header: " + line);
            if (line.contains(": ")) {
            ...
            if (!readingBoundary && line.contains(": ")) {
                ...
                if (headerVars.getProperty("Content-Type").contains("x-www-form-urlencoded")) {
                    ...
                    additionalData = line;
                    JavaCIPUnknownScope.logger.debug("HTTP Request Header Form Parameters: " + line);
                }
                ...
                if (method.equalsIgnoreCase("PUT")) {
                    if (line.contains("form-data; ")) {
                        ...
                        for (String varValue : formValues.replace("\"", "").split("; ")) {
                            ...
                            vV[0] = JavaCIPUnknownScope.decodePercent(vV[0]);
                            vV[1] = JavaCIPUnknownScope.decodePercent(vV[1]);
                ...
                if (headerVars.getProperty("Content-Transfer-Encoding").contains("binary")) {
                    ...
                    if (!uploadFilePath.exists()) {
                        JavaCIPUnknownScope.logger.error("Temporaty dir does not exist: " + uploadFilePath.getCanonicalPath());
                    }
                    if (!uploadFilePath.isDirectory()) {
                        JavaCIPUnknownScope.logger.error("Temporary dir is not a directory: " + uploadFilePath.getCanonicalPath());
                    }
                    if (!uploadFilePath.canWrite()) {
                        JavaCIPUnknownScope.logger.error("VOctopus Webserver doesn't have permissions to write on temporary dir: " + uploadFilePath.getCanonicalPath());
                    }
