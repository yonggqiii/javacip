    ...
    private void writeResponse(final Collection<? extends Resource> resources, final HttpServletResponse response) throws IOException {
        for (final Resource resource : resources) {
            ...
            try {
                ...
                if (bytesCopied < 0L)
                    throw new StreamCorruptedException("Bad number of copied bytes (" + bytesCopied + ") for resource=" + resource.getFilename());
                if (JavaCIPUnknownScope.logger.isDebugEnabled())
                    JavaCIPUnknownScope.logger.debug("writeResponse(" + resource.getFile() + ") copied " + bytesCopied + " bytes");
            } finally {
                if (in != null)
                    in.close();
            }
