class c14193058 {
    ...
    public static String submitURLRequest(String url) throws HttpException, IOException, URISyntaxException {
        ...
        JavaCIPUnknownScope.user_agents = new LinkedList<String>();
        JavaCIPUnknownScope.user_agents.add("Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.2.2) Gecko/20100316 Firefox/3.6.2");
        String response_text = "";
        URI uri = new URI(url);
        HttpGet post = new HttpGet(uri);
        int MAX = JavaCIPUnknownScope.user_agents.size() - 1;
        int index = (int) Math.round(((double) Math.random() * (MAX)));
        String agent = JavaCIPUnknownScope.user_agents.get(index);
        ...
        if (entity != null) {
            ...
            response_text = JavaCIPUnknownScope.convertStreamToString(stream);
        }
