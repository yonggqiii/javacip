...
class c21836608 {
    ...
    private void connect() throws RuntimeException {
        if (client != null) throw new IllegalStateRuntimeException("Already connected.");
        try {
            _logger.debug("About to connect to ftp server " + server + " port " + port);
            client = new FTPClient();
            client.connect(server, port);
            if (!FTPReply.isPositiveCompletion(client.getReplyCode())) {
                throw new RuntimeException("Unable to connect to FTP server " + server + " port " + port + " got error [" + client.getReplyString() + "]");
            }
            ...
            if (!client.login(username, password)) {
                throw new RuntimeException("Invalid username / password combination for FTP server " + server + " port " + port);
            }
            ...
            if (client.changeWorkingDirectory(remoteRootDir)) {
                ...
                if (client.makeDirectory(remoteRootDir)) {
                    ...
                    throw new RuntimeException("Cannot create directory [" + remoteRootDir + "] on FTP server " + server + " port " + port);
                }
            }
