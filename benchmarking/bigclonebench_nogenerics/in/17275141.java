...
class c17275141 {
    ...
    public static void copy(String sourceName, String destName, StatusWindow status) throws IORuntimeException {
        ...
        int bytes_read;
        long byteCount = 0;
        if (!src.exists()) throw new IORuntimeException("Source not found: " + src);
        if (!src.canRead()) throw new IORuntimeException("Source is unreadable: " + src);
        if (src.isFile()) {
            if (!dest.exists()) {
            ...
            if (dest.isFile()) throw new IORuntimeException("Cannot copy directory " + src + " to file " + dest);
            if (!dest.exists()) dest.mkdir();
        }
        if ((!dest.canWrite()) && (dest.exists())) throw new IORuntimeException("Destination is unwriteable: " + dest);
        if (src.isFile()) {
            try {
                if (status != null) {
