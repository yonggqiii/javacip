    ...
    final void saveProject(Project project, final File file) {
        if (projectsList.contains(project)) {
            if (project.isDirty() || !file.getParentFile().equals(workspaceDirectory)) {
                try {
                    if (!file.exists()) {
                        if (!file.createNewFile()) throw new IORuntimeException("cannot create file " + file.getAbsolutePath());
                    }
        ...
        } else throw new IllegalArgumentRuntimeException("The project " + project.getName() + " is not handled by this project set!");
    }
