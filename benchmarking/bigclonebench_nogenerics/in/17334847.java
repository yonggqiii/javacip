...
class c17334847 {
    ...
    private static MyCookieData parseCookie(Cookie cookie) throws CookieRuntimeException {
        ...
        if (parts.length < 4) throw new CookieRuntimeException("only " + parts.length + " parts in the cookie! " + value);
        ...
        if (cookie_secret == "") throw new CookieRuntimeException("cookie secret is not set");
        if (email.equals("")) {
