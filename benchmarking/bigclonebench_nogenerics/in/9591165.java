...
class c9591165 {
    ...
    private void writeAndCheckFile(DataFileReader reader, String base, String path, String hash, Reference ref, boolean hashall) throws RuntimeException {
        Data data = ref.data;
        ...
        if (hashall) {
            ...
            if (!hash.equals(filehash)) throw new RuntimeRuntimeException("hash wasn't equal for " + file);
        }
        file.setLastModified(ref.lastmod);
        if (file.length() != data.size) throw new RuntimeRuntimeException("corrupted file " + file);
    }
