class c7872668 {
    ...
    private static boolean computeCustomerAverages(String completePath, String CustomerAveragesOutputFileName, String CustIndexFileName) {
        try {
            File inputFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + CustIndexFileName);
            ...
            File outFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + CustomerAveragesOutputFileName);
            FileChannel outC = new FileOutputStream(outFile, true).getChannel();
            int totalCusts = CustomerLimitsTHash.size();
            File movieMMAPDATAFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + "MovieRatingBinaryFile.txt");
