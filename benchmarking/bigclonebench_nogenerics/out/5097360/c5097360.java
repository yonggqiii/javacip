class c5097360 {
    ...
    public static BufferedReader openForReading(String name, URI base, ClassLoader classLoader) throws IORuntimeException {
        ...
        if (name.trim().equals("System.in")) {
            if (JavaCIPUnknownScope.STD_IN == null) {
                JavaCIPUnknownScope.STD_IN = new BufferedReader(new InputStreamReader(System.in));
            }
            return JavaCIPUnknownScope.STD_IN;
        }
        URL url = JavaCIPUnknownScope.nameToURL(name, base, classLoader);
        if (url == null) {
