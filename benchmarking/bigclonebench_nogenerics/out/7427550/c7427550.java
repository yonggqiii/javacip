class c7427550 {
    ...
    public void process(int branch, int level) {
        ...
        try {
            ...
            conn.setRequestProperty("Cookie", JavaCIPUnknownScope.sessionId);
            ...
            while ((line = i.readLine()) != null) {
                ...
                m = p.matcher(line);
                if (m1.find()) {
                    if (m1.group(1).equals(JavaCIPUnknownScope.createIdent(level))) {
                        doAdd = true;
                    } else {
                        doAdd = false;
                    }
                ...
                if (m.find()) {
                    if (doAdd) {
                        JavaCIPUnknownScope.trainee.add(new Node(Integer.parseInt(m.group(1)), m.group(2)));
