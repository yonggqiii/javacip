class c17141933 {
    ...
    public static InputSource getInputSource(URL url) throws IORuntimeException {
        ...
        if (!("http".equals(proto) || "https".equals(proto)))
            throw new IllegalArgumentRuntimeException("OAI-PMH only allows HTTP(S) as network protocol!");
        ...
        JavaCIPUnknownScope.log.debug("Opening connection...");
        InputStream in = null;
        try {
            conn.connect();
            ...
            if (code == HttpURLConnection.HTTP_UNAVAILABLE && after > 0)
                throw new RetryAfterIORuntimeException(after, ioe);
            throw ioe;
        }
        ...
        JavaCIPUnknownScope.log.debug("HTTP server uses " + encoding + " content encoding.");
        if ("gzip".equals(encoding))
            in = new GZIPInputStream(in);
        else if ("deflate".equals(encoding))
            in = new InflaterInputStream(in);
        else if (!"identity".equals(encoding))
            throw new IORuntimeException("Server uses an invalid content encoding: " + encoding);
        ...
        JavaCIPUnknownScope.log.debug("Charset from Content-Type: '" + charset + "'");
