class c3298868 {
    ...
    public APIResponse update(Transaction transaction) throws RuntimeException {
        APIResponse response = new APIResponse();
        JavaCIPUnknownScope.connection = (HttpURLConnection) new URL(JavaCIPUnknownScope.url + "/api/transaction/update").openConnection();
        ...
        JavaCIPUnknownScope.marshaller.marshal(transaction, new MappedXMLStreamWriter(new MappedNamespaceConvention(new Configuration()), new OutputStreamWriter(JavaCIPUnknownScope.connection.getOutputStream(), "utf-8")));
        ...
        if (JavaCIPUnknownScope.connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
            ...
            response.setMessage("Update Transaction Error Code: Http (" + JavaCIPUnknownScope.connection.getResponseCode() + ")");
        }
