class c21093039 {
    ...
    public ArrayList parseFile(File newfile) throws IORuntimeException {
        String s;
        String firstname;
        String secondname;
        String direction;
        ...
        while ((s = br.readLine()) != null) {
            ...
            if (secondname.startsWith("BPM")) {
                if (readingHArrays)
                    JavaCIPUnknownScope.dumpxData(name, xturndat);
                else if (readingVArrays)
                    JavaCIPUnknownScope.dumpyData(name, yturndat);
                else if (readingAArrays)
                    JavaCIPUnknownScope.dumpampData(name, ampturndat);
                direction = tokens[4];
        ...
        JavaCIPUnknownScope.dumpampData(name, ampturndat);
        JavaCIPUnknownScope.data.add(JavaCIPUnknownScope.xdatamap);
        JavaCIPUnknownScope.data.add(JavaCIPUnknownScope.ydatamap);
        JavaCIPUnknownScope.data.add(JavaCIPUnknownScope.ampdatamap);
        return JavaCIPUnknownScope.data;
    }
}
