class c21161481 {
    ...
    public static void toZip(File zippedFile, File[] filesToZip, String zipComment, boolean savePath, int compressionLevel) throws IORuntimeException, FileNotFoundRuntimeException, ZipRuntimeException {
        if (zippedFile != null && filesToZip != null) {
            ...
            if (JavaCIPUnknownScope.ZIP_NO_COMPRESSION <= compressionLevel && compressionLevel <= JavaCIPUnknownScope.ZIP_MAX_COMPRESSION)
                out.setLevel(compressionLevel);
            else
                out.setLevel(JavaCIPUnknownScope.ZIP_MAX_COMPRESSION);
            if (zipComment != null)
            ...
            for (int i = 0; i < filesToZip.length; i++) {
                ...
                if (savePath) {
                    ...
                    out.putNextEntry(new ZipEntry(JavaCIPUnknownScope.cleanPath(filesToZip[i].getAbsolutePath())));
                } else {
                    in = new BufferedInputStream(new FileInputStream(filesToZip[i]));
            ...
            throw new ZipRuntimeException(JavaCIPUnknownScope.MAIN_RESOURCE_BUNDLE.getString("default.ZipRuntimeException.text"));
    }
}
