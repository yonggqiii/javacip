            ...
            if (!f.isDirectory()) {
                ...
                if (img != null && img.getStackSize() == 1) {
                    if (!JavaCIPUnknownScope.scoutengine(img))
                        return;
                    if (!JavaCIPUnknownScope.statustext) {
                        JavaCIPUnknownScope.IJ.log(list[i] + "/" + JavaCIPUnknownScope.whichprefix1 + "/" + JavaCIPUnknownScope.whichprefix2 + "/" + JavaCIPUnknownScope.whichprefix3 + "/" + JavaCIPUnknownScope.whichcase);
                    }
                    int lastDigit = JavaCIPUnknownScope.whichcase.length() - 1;
                    while (lastDigit > 0) {
                        if (!Character.isDigit(JavaCIPUnknownScope.whichcase.charAt(lastDigit)))
                            lastDigit -= 1;
                        else
                            break;
                    }
                    if (lastDigit < JavaCIPUnknownScope.whichcase.length() - 1)
                        JavaCIPUnknownScope.whichcase = JavaCIPUnknownScope.whichcase.substring(0, lastDigit + 1);
                    bubblesort[i] = Long.parseLong(JavaCIPUnknownScope.whichcase);
                }
            }
        }
