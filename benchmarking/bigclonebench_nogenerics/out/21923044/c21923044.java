    ...
    private String copy(PluginVersionDetail usePluginVersion, File runtimeRepository) {
        try {
            ...
            try {
                ...
                String tmpFileMessageDigest = JavaCIPUnknownScope.getMessageDigest(tmpFile.toURI().toURL()).getValue();
                if (!tmpFileMessageDigest.equals(usePluginVersion.getMessageDigest().getValue())) {
                    throw new RuntimeRuntimeException("Downloaded file: " + usePluginVersion.getUri() + " does not have required message digest: " + usePluginVersion.getMessageDigest().getValue());
                }
                if (!JavaCIPUnknownScope.isNoop()) {
            ...
            throw new RuntimeRuntimeException("Could not download " + usePluginVersion.getUri() + " to " + runtimeRepository, ex);
        }
    }
}
