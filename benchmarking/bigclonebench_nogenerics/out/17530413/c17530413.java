class c17530413 {
    ...
    protected void serveStaticContent(HttpServletRequest request, HttpServletResponse response, String pathInfo) throws ServletRuntimeException {
        InputStream is = JavaCIPUnknownScope.servletConfig.getServletContext().getResourceAsStream(pathInfo);
        if (is == null) {
        ...
        try {
            ...
            if (ind != -1 && ind < pathInfo.length()) {
                String type = JavaCIPUnknownScope.STATIC_CONTENT_TYPES.get(pathInfo.substring(ind + 1));
                if (type != null) {
