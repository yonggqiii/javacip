class c10443091 {
    ...
    public boolean validateZipFile(File zipFile) {
        String tempdir = Config.CONTEXT.getRealPath(JavaCIPUnknownScope.getBackupTempFilePath());
        try {
            JavaCIPUnknownScope.deleteTempFiles();
            ...
            if (zipFile != null && zipFile.getName().toLowerCase().endsWith(".zip")) {
                ...
                ZipUtil.extract(z, new File(Config.CONTEXT.getRealPath(JavaCIPUnknownScope.backupTempFilePath)));
            }
            return true;
