class c19053519 {
    ...
    protected String getManualDownloadURL() {
        if (JavaCIPUnknownScope._newestVersionString.indexOf("weekly") > 0) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if ((pos = line.indexOf(JavaCIPUnknownScope._newestVersionString)) >= 0) {
                    ...
                    if ((prePos >= 0) && (prePos < pos)) {
                        ...
                        if ((suffixPos >= 0) && (suffixPos + LINK_SUFFIX.length() == pos)) {
                            String versionLink = JavaCIPUnknownScope.edu.rice.cs.plt.text.TextUtil.xmlUnescape(line.substring(prePos + LINK_PREFIX.length(), suffixPos));
