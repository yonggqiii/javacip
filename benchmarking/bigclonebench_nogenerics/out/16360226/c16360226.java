class c16360226 {
    ...
    public static void main(String[] args) {
        ...
        if (method.equals("reloadpolicies")) {
            if (args.length == 4) {
                try {
                    JavaCIPUnknownScope.reloadPolicies(args[1], args[2], args[3]);
            ...
            try {
                boolean addXMLHeader = JavaCIPUnknownScope.getArgBoolean(args, 7, false);
                boolean reformat = JavaCIPUnknownScope.getArgBoolean(args, 8, false);
                boolean setMIMETypeCharset = JavaCIPUnknownScope.getArgBoolean(args, 9, false);
                ;
                InputStream is = JavaCIPUnknownScope.modifyDatastreamControlGroup(args[1], args[2], args[3], args[4], args[5], args[6], addXMLHeader, reformat, setMIMETypeCharset);
