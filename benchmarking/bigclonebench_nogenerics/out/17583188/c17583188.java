class c17583188 {
    ...
    byte[] toByteArray() throws IORuntimeException {
        DataElement element = new DataElement(DataElement.DATSEQ);
        final boolean sort = true;
        if (sort) {
            int[] sortIDs = new int[JavaCIPUnknownScope.attributes.size()];
            int k = 0;
            for (Enumeration e = JavaCIPUnknownScope.attributes.keys(); e.hasMoreElements(); ) {
            ...
            for (int i = 0; i < sortIDs.length; i++) {
                ...
                element.addElement(JavaCIPUnknownScope.getAttributeValue(sortIDs[i]));
            }
        } else {
            for (Enumeration e = JavaCIPUnknownScope.attributes.keys(); e.hasMoreElements(); ) {
                Integer key = (Integer) e.nextElement();
                element.addElement(new DataElement(DataElement.U_INT_2, key.intValue()));
                element.addElement((DataElement) JavaCIPUnknownScope.attributes.get(key));
            }
        }
