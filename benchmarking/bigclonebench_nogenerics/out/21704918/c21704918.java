class c21704918 {
    ...
    public void run(IProgressMonitor monitor) throws InvocationTargetRuntimeException, InterruptedRuntimeException {
        ...
        JavaCIPUnknownScope.LOGGER.info(message);
        monitor.beginTask(message, 1);
        HttpClient client = new HttpClient();
        HttpMethod method = new GetMethod(JavaCIPUnknownScope.DOWNLOAD_URI);
        ...
        if (statusCode != HttpStatus.SC_OK) {
            JavaCIPUnknownScope.LOGGER.error(MessageFormat.format("Can't download ffmpeg.zip from zourceforge, " + "status = [{0}]", statusCode));
            monitor.setCanceled(true);
            return;
        }
        monitor.worked(1);
        ...
        monitor.beginTask("Downloading FFMpeg, " + JavaCIPUnknownScope.DOWNLOAD_URI, (int) contentLength);
        ...
        try {
            ...
            OutputStream output = new FileOutputStream(JavaCIPUnknownScope.target);
