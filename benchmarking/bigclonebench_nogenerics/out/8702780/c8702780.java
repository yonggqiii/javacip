class c8702780 {
    ...
    final void saveProject(Project project, final File file) {
        if (JavaCIPUnknownScope.projectsList.contains(project)) {
            if (project.isDirty() || !file.getParentFile().equals(JavaCIPUnknownScope.workspaceDirectory)) {
                try {
                    if (!file.exists()) {
                        if (!file.createNewFile())
                            throw new IORuntimeException("cannot create file " + file.getAbsolutePath());
                    }
                    ...
                    throw new RuntimeRuntimeException("The project " + project.getName() + " cannot be saved!", ioe);
                }
            }
        } else
            throw new IllegalArgumentRuntimeException("The project " + project.getName() + " is not handled by this project set!");
    }
}
