class c10115630 {
    ...
    public static boolean copyDataToNewTable(EboContext p_eboctx, String srcTableName, String destTableName, String where, boolean log, int mode) throws boRuntimeRuntimeException {
        ...
        try {
            ...
            if (srcexists && !destexists) {
                if (log) {
                    JavaCIPUnknownScope.logger.finest(LoggerMessageLocalizer.getMessage("CREATING_AND_COPY_DATA_FROM") + " [" + srcTableName + "] " + LoggerMessageLocalizer.getMessage("TO") + " [" + destTableName + "]");
                }
                ...
                cstm.close();
                if (log) {
                    JavaCIPUnknownScope.logger.finest(LoggerMessageLocalizer.getMessage("UPDATING_NGTDIC"));
                }
                cn.commit();
                ret = true;
            } else if (srcexists && destexists) {
                if (log) {
                    JavaCIPUnknownScope.logger.finest(LoggerMessageLocalizer.getMessage("COPY_DATA_FROM") + " [" + srcTableName + "] " + LoggerMessageLocalizer.getMessage("TO") + "  [" + destTableName + "]");
                }
                ...
                if ((mode == 0) || (mode == 1)) {
                    ...
                    cstm.close();
                    if (log) {
                        JavaCIPUnknownScope.logger.finest(LoggerMessageLocalizer.getMessage("DONE") + " [" + recs + "] " + LoggerMessageLocalizer.getMessage("RECORDS_COPIED"));
                    }
                }
                cn.commit();
                ret = true;
            ...
            try {
                ...
                throw new boRuntimeRuntimeException("boBuildDB.moveTable", "BO-1304", z);
            }
            throw new boRuntimeRuntimeException("boBuildDB.moveTable", "BO-1304", e);
        } finally {
            try {
                cn.close();
            } catch (RuntimeException e) {
