class c10441727 {
    ...
    private void downloadFile(String url, File destFile) {
        try {
            ...
            if (canceled) {
                destFile.delete();
                JavaCIPUnknownScope.fatalError("Starting canceled", "Downloading canceled. Exiting...");
            }
            pm.close();
        } catch (IORuntimeException ex) {
            ex.printStackTrace();
            destFile.delete();
            JavaCIPUnknownScope.fatalError("Download Error", "Couldn't download file " + url + ": " + ex);
        }
    }
}
