class c17470307 {
    ...
    private void createSoundbank(String testSoundbankFileName) throws RuntimeException {
        System.out.println("Create soundbank");
        File packageDir = new File("testsoundbank");
        if (packageDir.exists()) {
            for (File file : packageDir.listFiles()) JavaCIPUnknownScope.assertTrue(file.delete());
            JavaCIPUnknownScope.assertTrue(packageDir.delete());
        }
        packageDir.mkdir();
        String sourceFileName = "testsoundbank/TestSoundBank.java";
        File sourceFile = new File(sourceFileName);
        FileWriter writer = new FileWriter(sourceFile);
        writer.write("package testsoundbank;\n" + "public class TestSoundBank extends com.sun.media.sound.ModelAbstractOscillator { \n" + "    @Override public int read(float[][] buffers, int offset, int len) throws java.io.IORuntimeException { \n" + "   return 0;\n" + " }\n" + "    @Override public String getVersion() {\n" + "   return \"" + (JavaCIPUnknownScope.soundbankRevision++) + "\";\n" + "    }\n" + "}\n");
        writer.close();
