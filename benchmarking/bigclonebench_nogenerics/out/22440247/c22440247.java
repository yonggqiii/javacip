class c22440247 {
    ...
    private void serviceResource(HttpServletRequest request, HttpServletResponse response, ServletConfig config) throws IORuntimeException {
        String uri = (String) request.getAttribute("javax.servlet.include.request_uri");
        if (uri == null)
            uri = request.getRequestURI();
        if (uri.endsWith(".shtml")) {
            JavaCIPUnknownScope.serviceSSIResource(uri, response, config);
        } else {
            String target = uri.substring(JavaCIPUnknownScope.servletPath.length());
