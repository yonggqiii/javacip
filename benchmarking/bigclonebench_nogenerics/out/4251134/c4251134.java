class c4251134 {
    ...
    protected URLConnection openConnection(URL url) throws IORuntimeException {
        if (url.getQuery() == null)
            throw new IllegalStateRuntimeException("Missing TemplateAccount number in rest URL " + url);
        MSResource msResource = null;
        try {
            long templateAccountId = Long.parseLong(url.getQuery());
            msResource = JavaCIPUnknownScope.menuBean.findMSResource(templateAccountId, url.getPath());
