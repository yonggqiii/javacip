class c8063675 {
    ...
    private static int ejecutaUpdate(String database, String SQL) throws RuntimeException {
        ...
        try {
            ...
            JavaCIPUnknownScope.log.error("SQL error: " + SQL, e);
            RuntimeException excep;
            if (dbm == null)
                excep = new RuntimeException("Could not obtain pool object DbConnectionManager");
            else if (bd == null)
                excep = new RuntimeException("The Db connection pool could not obtain a database connection");
            else {
                bd.rollback();
