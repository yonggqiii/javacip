    ...
    public static boolean predictDataSet(String completePath, String Type, String predictionOutputFileName, String slopeOneDataFolderName) {
        try {
            if (Type.equalsIgnoreCase("Qualifying")) {
                File inputFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + "CompleteQualifyingDataInByteFormat.txt");
                ...
                File outFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + predictionOutputFileName);
                ...
                for (int i = 0; i < movies.length; i++) {
                    ...
                    movieDiffStats = JavaCIPUnknownScope.loadMovieDiffStats(completePath, movieToProcess, slopeOneDataFolderName);
                    ...
                    for (int j = 0; j < customersToProcess.size(); j++) {
                        ...
                        finalPrediction = JavaCIPUnknownScope.predictSlopeOneRating(customerToProcess, movieDiffStats);
                        if (finalPrediction == finalPrediction) {
                            if (finalPrediction < 1.0)
                                finalPrediction = 1.0;
                            else if (finalPrediction > 5.0)
                                finalPrediction = 5.0;
                        } else
                            finalPrediction = JavaCIPUnknownScope.GetAveragePrediction(movieToProcess);
                ...
                File inputFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + "CompleteProbeDataInByteFormat.txt");
                FileChannel inC = new FileInputStream(inputFile).getChannel();
                int filesize = (int) inC.size();
