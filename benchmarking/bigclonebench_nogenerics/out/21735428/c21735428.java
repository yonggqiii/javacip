class c21735428 {
    ...
    private DictionaryListParser downloadList(final String url) throws IORuntimeException, JSONRuntimeException {
        ...
        final boolean isValidType = entity.getContentType().getValue().startsWith(JavaCIPUnknownScope.RESPONSE_CONTENT_TYPE);
        if (!isValidType) {
            final String message = "CONTENT_TYPE IS '" + entity.getContentType().getValue() + "'";
            throw new IORuntimeException(message);
        }
        final BufferedReader reader = new BufferedReader(new InputStreamReader(entity.getContent(), JavaCIPUnknownScope.RESPONSE_ENCODING));
