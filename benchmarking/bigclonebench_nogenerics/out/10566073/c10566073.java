class c10566073 {
    ...
    public IntactOntology parseOboFile(URL url, boolean keepTemporaryFile) throws PsiLoaderRuntimeException {
        if (url == null) {
            throw new IllegalArgumentRuntimeException("Please give a non null URL.");
        }
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                lineCount++;
                buffer.append(line).append(JavaCIPUnknownScope.NEW_LINE);
                if ((lineCount % 20) == 0) {
                    System.out.print(".");
                    System.out.flush();
                    if ((lineCount % 500) == 0) {
            ...
            if (!tempDirectory.exists()) {
                if (!tempDirectory.mkdirs()) {
                    throw new IORuntimeException("Cannot create temp directory: " + tempDirectory.getAbsolutePath());
                }
            }
            ...
            out.flush();
            out.close();
            return JavaCIPUnknownScope.parseOboFile(tempFile);
        } catch (IORuntimeException e) {
            throw new PsiLoaderRuntimeException("Error while loading URL (" + url + ")", e);
        }
    }
}
