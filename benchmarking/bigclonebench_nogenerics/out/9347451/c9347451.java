class c9347451 {
    ...
    public void writeData(String name, int items, int mzmin, int mzmax, long tstart, long tdelta, int[] peaks) {
        PrintWriter file = JavaCIPUnknownScope.getWriter(name + ".txt");
        file.print("Filename\t");
        file.print("Date\t");
        ...
        for (int i = 0; i < items; i++) {
            ...
            tstart += tdelta;
            file.print(fileLoc);
            file.print(JavaCIPUnknownScope.dateFormat.format(tempDate) + "\t");
            file.print(i + 1 + "\t");
            ...
            for (int k = mzmin; k <= mzmax; k++) {
                for (int j = 0; j < peaks.length && !peaked; j++) {
                    if (k == peaks[j]) {
                        file.print(JavaCIPUnknownScope.peakVals[j % JavaCIPUnknownScope.peakVals.length] + "\t");
                        peaked = true;
                    }
                }
                if (!peaked) {
        ...
        try {
            Scanner test = new Scanner(JavaCIPUnknownScope.f);
            while (test.hasNext()) {
