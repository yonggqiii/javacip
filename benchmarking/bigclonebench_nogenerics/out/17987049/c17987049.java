class c17987049 {
    ...
    private boolean writeResource(PluginProxy eclipseInstallPlugin, ResourceProxy translation, LocaleProxy locale) throws RuntimeException {
        String translationResourceName = JavaCIPUnknownScope.determineTranslatedResourceName(translation, locale);
        ...
        if (translation.getRelativePath().contains(pluginNameInDirFormat)) {
            return JavaCIPUnknownScope.writeResourceToBundleClasspath(translation, locale);
            ...
            File resourcePathDirectory = new File(JavaCIPUnknownScope.directory.getPath() + File.separatorChar + resourcePath);
            resourcePathDirectory.mkdirs();
        }
        File fragmentResource = new File(JavaCIPUnknownScope.directory.getPath() + File.separatorChar + translationResourceName);
