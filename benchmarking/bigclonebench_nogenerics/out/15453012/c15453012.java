    ...
    public ArrayList parseFile(File newfile) throws IORuntimeException {
        ...
        while ((s = br.readLine()) != null) {
            ...
            if (firstName.indexOf("WS") > 0) {
                if (name != null) {
                    JavaCIPUnknownScope.dumpData(name, fitparams, sraw, sxraw, syraw, szraw, yraw, zraw, xraw);
                }
                name = tokens[0];
                readraw = false;
                readfit = false;
        ...
        JavaCIPUnknownScope.dumpData(name, fitparams, sraw, sxraw, syraw, szraw, yraw, zraw, xraw);
        JavaCIPUnknownScope.wiredata.add((Integer) PVLoggerID);
        return JavaCIPUnknownScope.wiredata;
    }
}
