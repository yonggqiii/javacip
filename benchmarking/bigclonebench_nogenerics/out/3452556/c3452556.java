class c3452556 {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IORuntimeException, ServletRuntimeException {
        ...
        PrintWriter out = JavaCIPUnknownScope.helper.getPrintWriter(resp);
        MessageBundle messageBundle = new MessageBundle("net.sf.warpcore.cms/servlets/InjectorServletMessages");
        Locale locale = req.getLocale();
        JavaCIPUnknownScope.helper.header(out, messageBundle, locale);
        if (git.getUser() == null) {
            JavaCIPUnknownScope.helper.notLoggedIn(out, messageBundle, locale);
        } else {
            try {
        ...
        JavaCIPUnknownScope.helper.footer(out);
    }
}
