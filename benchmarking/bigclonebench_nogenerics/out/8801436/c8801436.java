class c8801436 {
    ...
    public static void copy(File from_file, File to_file) throws IORuntimeException {
        if (!from_file.exists())
            JavaCIPUnknownScope.abort("FileCopy: no such source file: " + from_file.getName());
        if (!from_file.isFile())
            JavaCIPUnknownScope.abort("FileCopy: can't copy directory: " + from_file.getName());
        if (!from_file.canRead())
            JavaCIPUnknownScope.abort("FileCopy: source file is unreadable: " + from_file.getName());
