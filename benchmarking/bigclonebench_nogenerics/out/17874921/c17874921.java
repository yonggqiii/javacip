class c17874921 {
    ...
    public static boolean buildCFItem2ItemStats(String outFileName, String movieAvgFileName, String custAvgFileName) {
        try {
            File infile = new File(JavaCIPUnknownScope.completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + movieAvgFileName);
            ...
            map = null;
            infile = new File(JavaCIPUnknownScope.completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + custAvgFileName);
            ...
            File outfile = new File(JavaCIPUnknownScope.completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + outFileName);
            ...
            for (int i = 0; i < noMovies - 1; i++) {
                ...
                for (int j = i + 1; j < noMovies; j++) {
                    ...
                    TIntArrayList intersectSet = JavaCIPUnknownScope.CustOverLapForTwoMoviesCustom(customers1, customers2);
                    int count = 0;
                    float diffRating = 0;
                    float pearsonCorr = 0;
                    float cosineCorr = 0;
