class c18427451 {
    ...
    static void cleanFile(File file) {
        ...
        if (!it.hasNext())
            return;
        JavaCIPUnknownScope.log.info("Cleaning file " + file + " to " + out.getName());
        ...
        while (it.hasNext()) {
            ...
            if (start1 == start2 && end1 == end2) {
                JavaCIPUnknownScope.log.debug("Discarding record " + now.toString());
                cnt.count();
                continue;
            }
        ...
        writer.close();
        reader.close();
        JavaCIPUnknownScope.log.info(file + " done, discarded " + cnt.getCount() + " reads");
        JavaCIPUnknownScope.exe.shutdown();
    }
}
