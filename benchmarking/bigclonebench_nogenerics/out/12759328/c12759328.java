class c12759328 {
    ...
    final void importSelectedEntity() {
        TreePath path = JavaCIPUnknownScope.projectsTree.getSelectionPath();
        ...
        if (userObject instanceof ProjectSet) {
            ...
            int result = fileChooser.showOpenDialog(JavaCIPUnknownScope.mainWindow.getApplicativeFrame());
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                if (Arrays.asList(workspace.getProjectNames()).contains(projectName)) {
                    confirmLoad = JOptionPane.showConfirmDialog(JavaCIPUnknownScope.mainWindow.getApplicativeFrame(), "Overwrite project \"" + projectName + "\"" + "?", "Overwrite project", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION;
                    if (confirmLoad) {
                        ...
                        for (FloorSet floorSet : project.getLevelSet().getFloorSetsList()) for (Floor floor : floorSet.getFloorsList()) JavaCIPUnknownScope.mainWindow.getEntityViewer().closeEntityView(floor);
                        for (Tile tile : project.getTileSet().getTilesList()) JavaCIPUnknownScope.mainWindow.getEntityViewer().closeEntityView(tile);
                        workspace.removeProject(project);
                        ((DefaultTreeModel) JavaCIPUnknownScope.projectsTree.getModel()).removeNodeFromParent(projectNode);
                    }
                }
                if (confirmLoad) {
                    ...
                    try {
                        ...
                        JavaCIPUnknownScope.displayErrorMessage(throwable, false);
                        success = false;
                    }
                    if (success)
                        JavaCIPUnknownScope.addProject(projectName);
                }
            }
            ...
            JavaCIPUnknownScope.importImageForSelectedMap(floor, map);
        }
    }
}
