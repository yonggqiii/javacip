class c10158742 {
    ...
    public void moveRuleDown(String language, String tag, int row) throws FidoDatabaseRuntimeException {
        try {
            ...
            try {
                ...
                int max = JavaCIPUnknownScope.findMaxRank(stmt, language, tag);
                if ((row < 1) || (row > (max - 1)))
                    throw new IllegalArgumentRuntimeException("Row number (" + row + ") was not between 1 and " + (max - 1));
