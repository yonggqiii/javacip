class c11238287 {
    ...
    public static void main(final String[] args) throws RecognitionRuntimeException, TokenStreamRuntimeException, IORuntimeException, IllegalOptionValueRuntimeException, UnknownOptionRuntimeException {
        try {
            ...
            if (remainingArgs.length != 2) {
                JavaCIPUnknownScope.printUsage("Input and output file are not specified correctly. ");
            }
            File inputFile = new File(remainingArgs[0]);
            if (!inputFile.exists()) {
                JavaCIPUnknownScope.printUsage("Input file " + remainingArgs[0] + " does not exist. ");
            }
            ...
            if ("tex".equals(format)) {
                ...
                JavaCIPUnknownScope.printUsage("Format not specified via argument -f. Also guessing for the extension of output file " + outputFile.getName() + " failed");
            }
        } catch (RuntimeException ex) {
            ex.printStackTrace();
            JavaCIPUnknownScope.printUsage(ex.getMessage());
        }
    }
}
