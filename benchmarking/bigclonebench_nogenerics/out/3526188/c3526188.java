class c3526188 {
    ...
    public static FTPClient createConnection(String hostname, int port, char[] username, char[] password, String workingDirectory, FileSystemOptions fileSystemOptions) throws FileSystemRuntimeException {
        if (username == null) {
        ...
        try {
            ...
            try {
                ...
                if (!FTPReply.isPositiveCompletion(reply)) {
                    throw new FileSystemRuntimeException("vfs.provider.ftp/connect-rejected.error", hostname);
                }
                if (!client.login(UserAuthenticatorUtils.toString(username), UserAuthenticatorUtils.toString(password))) {
                    throw new FileSystemRuntimeException("vfs.provider.ftp/login.error", new Object[] { hostname, UserAuthenticatorUtils.toString(username) }, null);
                }
                if (!client.setFileType(JavaCIPUnknownScope.FTP.BINARY_FILE_TYPE)) {
                    throw new FileSystemRuntimeException("vfs.provider.ftp/set-binary.error", hostname);
                }
                ...
                if (workingDirectory != null && (userDirIsRoot == null || !userDirIsRoot.booleanValue())) {
                    if (!client.changeWorkingDirectory(workingDirectory)) {
                        throw new FileSystemRuntimeException("vfs.provider.ftp/change-work-directory.error", workingDirectory);
                    }
                }
            ...
            throw new FileSystemRuntimeException("vfs.provider.ftp/connect.error", new Object[] { hostname }, exc);
        }
    }
}
