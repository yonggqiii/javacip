class c2396194 {
    ...
    public static Vector getKeywordsFromURLFast(String p_url) throws RuntimeException {
        ...
        if (x_bod == false) {
            if (x_title_line != null) {
                x_words = new Vector();
                x_temp_words = JavaCIPUnknownScope.extractWordsFromSpacedList(x_title_line);
        ...
        do {
            ...
            if (x_end != -1) {
                JavaCIPUnknownScope.extractOutsideTextFast(x_line.substring(0, x_end), '<', '>', x_words, x_status, x_buf);
                break;
            }
            x_status = JavaCIPUnknownScope.extractOutsideTextFast(x_line, '<', '>', x_words, x_status, x_buf);
        } while ((x_line = x_reader.readLine()) != null);
        if (x_title_line != null)
            x_words.addElement(x_title_line);
        return x_words;
