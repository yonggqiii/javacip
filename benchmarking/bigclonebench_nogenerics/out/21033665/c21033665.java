class c21033665 {
    ...
    private boolean addBookmark0(Bookmark bookmark, BookmarkFolder folder, PreparedStatement preparedStatement) throws SQLRuntimeException {
        ...
        JavaCIPUnknownScope.embeddedConnection.setAutoCommit(false);
        PreparedStatement statement = null;
        try {
            if (preparedStatement == null) {
                statement = JavaCIPUnknownScope.embeddedConnection.prepareStatement(JavaCIPUnknownScope.BOOKMARK_INSERT);
            } else {
            ...
            try {
                ...
                long identityValue = JavaCIPUnknownScope.getInsertedPrimaryKey();
                bookmark.setId(identityValue);
                JavaCIPUnknownScope.addBindVariables(bookmark);
                hasError = false;
                return affectedCount == 1;
            } catch (SQLRuntimeException exception) {
                if (JavaCIPUnknownScope.CONSTRAINT_VIOLATION.equals(exception.getSQLState())) {
                    return false;
                }
                throw exception;
            }
            ...
            if (hasError) {
                JavaCIPUnknownScope.embeddedConnection.rollback();
            } else {
                JavaCIPUnknownScope.embeddedConnection.commit();
            }
            JavaCIPUnknownScope.embeddedConnection.setAutoCommit(true);
            if (preparedStatement != null) {
                if (!doBatch) {
                    try {
