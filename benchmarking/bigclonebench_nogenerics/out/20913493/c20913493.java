class c20913493 {
    ...
    public static void copy(File src, File dest) throws IORuntimeException {
        JavaCIPUnknownScope.log.info("Copying " + src.getAbsolutePath() + " to " + dest.getAbsolutePath());
        if (!src.exists())
            throw new IORuntimeException("File not found: " + src.getAbsolutePath());
        if (!src.canRead())
            throw new IORuntimeException("Source not readable: " + src.getAbsolutePath());
        if (src.isDirectory()) {
            if (!dest.exists())
                if (!dest.mkdirs())
                    throw new IORuntimeException("Could not create direcotry: " + dest.getAbsolutePath());
