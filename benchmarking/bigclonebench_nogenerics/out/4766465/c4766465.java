class c4766465 {
    ...
    private void saveVideo(URL url, DownloadTask task) {
        ...
        JavaCIPUnknownScope.publish("Starting download to file:" + file.getAbsolutePath());
        try {
            ...
            JavaCIPUnknownScope.publish("Content type: " + contentType + " with a length of " + length);
            ...
            while ((nextbyte = bis.read()) != -1) {
                ...
                if (System.currentTimeMillis() - time >= 1000) {
                    JavaCIPUnknownScope.publish("downloaded " + (i++) + " bytes of " + length + " bytes.");
                    time = System.currentTimeMillis();
                    int progress = (i - 1) / length;
                    if (progress <= 100 && progress >= 0)
                        JavaCIPUnknownScope.setProgress(progress);
                }
            }
            bos.write(bytes);
