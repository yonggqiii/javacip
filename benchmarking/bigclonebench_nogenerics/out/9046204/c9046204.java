class c9046204 {
    ...
    public static void makeLPKFile(String[] srcFilePath, String makeFilePath, LPKHeader header) {
        ...
        try {
            ...
            long fileOffset = JavaCIPUnknownScope.outputOffset(header);
            for (int i = 0; i < srcFilePath.length; i++) {
                ...
                LPKTable ft = JavaCIPUnknownScope.makeLPKTable(sourceFileName, sourceFileSize, fileOffset);
                fileOffset = JavaCIPUnknownScope.outputNextOffset(sourceFileSize, fileOffset);
                fileTable[i] = ft;
            }
            ...
            JavaCIPUnknownScope.writeByteArray(header.getPassword(), dos);
            dos.writeFloat(header.getVersion());
            dos.writeLong(header.getTables());
            for (int i = 0; i < fileTable.length; i++) {
                JavaCIPUnknownScope.writeByteArray(fileTable[i].getFileName(), dos);
            ...
            for (int i = 0; i < fileTable.length; i++) {
                ...
                while ((readLength = ftDis.read(buff)) != -1) {
                    JavaCIPUnknownScope.makeBuffer(buff, readLength);
