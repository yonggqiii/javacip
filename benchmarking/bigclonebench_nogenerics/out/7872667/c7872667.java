class c7872667 {
    ...
    private static boolean computeMovieAverages(String completePath, String MovieAveragesOutputFileName, String MovieIndexFileName) {
        try {
            File inputFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + MovieIndexFileName);
            ...
            File outFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + MovieAveragesOutputFileName);
            FileChannel outC = new FileOutputStream(outFile, true).getChannel();
            int totalMovies = MovieLimitsTHash.size();
            File movieMMAPDATAFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + "CustomerRatingBinaryFile.txt");
