    ...
    private void downloadPage(final URL url, final File file) {
        try {
            ...
            do {
                ...
                if (lastUpdate > JavaCIPUnknownScope.UPDATE_TIME) {
                    JavaCIPUnknownScope.report(0, (int) (size / Format.MEMORY_MEG), "Downloading... " + Format.formatMemory(size));
                    lastUpdate = 0;
                }
                lastUpdate++;
            } while (length >= 0);
            ...
            if (url.toString().toLowerCase().endsWith(".gz")) {
                ...
                do {
                    ...
                    if (lastUpdate > JavaCIPUnknownScope.UPDATE_TIME) {
                        JavaCIPUnknownScope.report(0, (int) (size / Format.MEMORY_MEG), "Uncompressing... " + Format.formatMemory(size));
                        lastUpdate = 0;
                    }
                    lastUpdate++;
                } while (length >= 0);
