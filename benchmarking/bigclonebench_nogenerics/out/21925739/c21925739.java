class c21925739 {
    ...
    public static void buildPerMovieDiffBinary(String completePath, String slopeOneDataFolderName, String slopeOneDataFileName) {
        try {
            File inFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + slopeOneDataFolderName + JavaCIPUnknownScope.fSep + slopeOneDataFileName);
            FileChannel inC = new FileInputStream(inFile).getChannel();
            for (int i = 1; i <= 17770; i++) {
                File outFile = new File(completePath + JavaCIPUnknownScope.fSep + "SmartGRAPE" + JavaCIPUnknownScope.fSep + slopeOneDataFolderName + JavaCIPUnknownScope.fSep + "Movie-" + i + "-SlopeOneData.txt");
