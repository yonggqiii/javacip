class c14001795 {
    ...
    public static void copyOverWarFile() {
        ...
        File dir = new File(JavaCIPUnknownScope.theAppsDataDir);
        ...
        if (dir.listFiles(ff).length == 0) {
            ...
            if (dir.exists()) {
                ...
                for (File f : files) {
                    try {
                        File newFile = new File("" + JavaCIPUnknownScope.theAppsDataDir + "/" + f.getName());
                        System.out.println("Creating new file \"" + f.getAbsolutePath() + "\"");
                        newFile.createNewFile();
                        InputStream fi = new FileInputStream(f);
                        OutputStream fo = new FileOutputStream(newFile);
                        IOUtils.copy(fi, fo);
                        JavaCIPUnknownScope.moveUnzipAndExtract(newFile);
        ...
        new JFileChooser().setCurrentDirectory(new File(JavaCIPUnknownScope.theAppsDataDir));
        System.setProperty("user.dir", JavaCIPUnknownScope.theAppsDataDir);
