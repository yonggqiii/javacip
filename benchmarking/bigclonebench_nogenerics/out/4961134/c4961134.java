class c4961134 {
    ...
    private void alterarArtista(Artista artista) throws RuntimeException {
        ...
        try {
            ...
            ps.executeUpdate();
            JavaCIPUnknownScope.alterarEndereco(conn, ps, artista);
            JavaCIPUnknownScope.delObras(conn, ps, artista.getNumeroInscricao());
            sql = "insert into obra VALUES (?,?,?,?,?,?)";
            ps = conn.prepareStatement(sql);
            for (Obra obra : artista.getListaObras()) {
                JavaCIPUnknownScope.salvarObra(conn, ps, obra, artista.getNumeroInscricao());
            }
            conn.commit();
