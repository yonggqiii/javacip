class c21817955 {
    ...
    public void movePrior(String[] showOrder, String[] orgID, String targetShowOrder, String targetOrgID) throws RuntimeException {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet result = null;
        int moveCount = showOrder.length;
        DBOperation dbo = JavaCIPUnknownScope.factory.createDBOperation(JavaCIPUnknownScope.POOL_NAME);
        ...
        try {
            ...
            for (int j = 0; j < sqls.length; j++) {
                ...
                if (resultCount != 1) {
                    throw new CesSystemRuntimeException("Organize.movePrior(): ERROR Inserting data " + "in T_SYS_ORGANIZE update !! resultCount = " + resultCount);
                }
            }
            con.commit();
            ...
            throw new CesSystemRuntimeException("Organize.movePrior(): SQLRuntimeException while mov organize order " + " :\n\t" + se);
        } finally {
            con.setAutoCommit(true);
            JavaCIPUnknownScope.close(dbo, ps, result);
        }
    }
}
