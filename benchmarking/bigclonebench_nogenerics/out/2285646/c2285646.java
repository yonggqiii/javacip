class c2285646 {
    ...
    protected void sendDoc(File indir, File outdir, File orig, Document doc, ServiceEndpoint ep) {
        ...
        try {
            response = JavaCIPUnknownScope.protocolHandler.sendMessage(ep, doc);
        } catch (TransportRuntimeException e) {
            JavaCIPUnknownScope.logger.warn("Message was not accepted, will try again later");
            return;
        }
        ...
        try {
            ...
            JavaCIPUnknownScope.serializeDocument(respos, response);
        } catch (IORuntimeException e) {
            JavaCIPUnknownScope.logger.warn("Failed to dump response");
            return;
        } finally {
            try {
                respos.close();
        ...
        try {
            ...
            JavaCIPUnknownScope.logger.warn("Failed to copy original");
            return;
        } finally {
            try {
                in.close();
        ...
        orig.delete();
        JavaCIPUnknownScope.logger.info("File processed: " + orig.getName());
    }
}
