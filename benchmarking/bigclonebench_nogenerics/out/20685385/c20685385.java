class c20685385 {
    ...
    private Long getNextPkValueForEntityIncreaseBy(String entityName, int count, int increasePkBy) {
        ...
        if (false) {
            EOEditingContext ec = JavaCIPUnknownScope.ERXEC.newEditingContext();
            ec.lock();
            try {
                ...
                for (int tries = 0; tries < count; tries++) {
                    try {
                        ...
                        if (hasNext) {
                            ...
                            pk = JavaCIPUnknownScope.maxIdFromTable(entityName);
                        ...
                        if (creationError) {
                            try {
                                con.rollback();
                                JavaCIPUnknownScope.log.info("creating pk table");
                                ...
                                con.commit();
                            } catch (SQLRuntimeException ee) {
                                throw new NSForwardRuntimeException(ee, "could not create pk table");
                            }
                        } else {
                            throw new NSForwardRuntimeException(ex, "Error fetching PK");
                        }
                    }
                }
            } finally {
        ...
        throw new IllegalStateRuntimeException("Couldn't get PK");
    }
}
