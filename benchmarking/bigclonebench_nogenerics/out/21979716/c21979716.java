class c21979716 {
    ...
    public void download() throws IORuntimeException {
        ...
        final int result = JOptionPane.showConfirmDialog(JavaCIPUnknownScope.frame, message, "Download parameters", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
        ...
        final URL url = new URL("http://127.0.0.1:" + JavaCIPUnknownScope.testPort + "/databases/" + fullname + "?device=" + device + "&version=" + version);
        ...
        try {
            final int fileResult = JavaCIPUnknownScope.fileChooser.showSaveDialog(JavaCIPUnknownScope.frame);
            if (fileResult != JFileChooser.APPROVE_OPTION) {
                return;
            }
            final OutputStream out = new FileOutputStream(JavaCIPUnknownScope.fileChooser.getSelectedFile());
            try {
                Util.copy(in, out);
            } finally {
                out.close();
