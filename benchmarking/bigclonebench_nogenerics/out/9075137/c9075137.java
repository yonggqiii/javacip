class c9075137 {
    ...
    public boolean implies(Permission permission) {
        if (!JavaCIPUnknownScope.permissionClass.isInstance(permission)) {
            return false;
        }
        GCFPermission perm = (GCFPermission) permission;
        int perm_low = perm.getMinPort();
        int perm_high = perm.getMaxPort();
        Enumeration search = JavaCIPUnknownScope.permissions.elements();
        int count = JavaCIPUnknownScope.permissions.size();
