class c4697215 {
    ...
    public void loadJar(final String extName, final String url, final String fileName, final IProgressListener pl) throws RuntimeException {
        pl.setName(fileName);
        pl.setProgress(0);
        pl.setFinished(false);
        pl.setStarted(true);
        String installDirName = JavaCIPUnknownScope.extDir + File.separator + extName;
        Log.log("extension installation directory: " + installDirName);
        File installDir = new File(installDirName);
        if (!installDir.exists()) {
            if (!installDir.mkdirs()) {
                throw new RuntimeException("ExtensionLoader.loadJar: Cannot create install directory: " + installDirName);
            }
        }
        ...
        if (jarFile.exists()) {
            ...
            if (indexFile.exists()) {
                ...
                long cachedTimeStamp = JavaCIPUnknownScope.readTimeStamp(indexFile);
        ...
        if (isDirty) {
            ...
            JavaCIPUnknownScope.downloadJar(downloadURL, jarFile, pl);
            indexFile = new File(installDir, indexFileName);
            Log.log("writing timestamp to index file");
            JavaCIPUnknownScope.writeTimeStamp(indexFile, urlTimeStamp);
        }
        JavaCIPUnknownScope.addJar(jarFile);
    }
}
