    ...
    public static String loadWebsiteHtmlCode(String url, String useragent) {
        ...
        try {
            ...
            if (statusCode != HttpStatus.SC_OK) {
                JavaCIPUnknownScope.logger.debug("Method failed!" + statusCode);
            }
            htmlCode = EntityUtils.toString(resp.getEntity());
        } catch (RuntimeException e) {
            JavaCIPUnknownScope.logger.debug("Fatal protocol violation: " + e.getMessage());
            JavaCIPUnknownScope.logger.trace(e);
        }
        return htmlCode;
    }
}
