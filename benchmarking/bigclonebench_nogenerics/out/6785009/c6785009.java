class c6785009 {
    ...
    private String[] sendRequest(String url, String requestString) throws ClickatellRuntimeException, IORuntimeException {
        String response = null;
        MessageFormat responseFormat = new MessageFormat("{0}: {1}");
        List idList = new LinkedList();
        try {
            JavaCIPUnknownScope.log_.debug("sendRequest: posting : " + requestString + " to " + url);
            ...
            while ((response = responseReader.readLine()) != null) {
                ...
                if ("ERR".equalsIgnoreCase((String) objs[0])) {
                    ...
                    throw new ClickatellRuntimeException("Clickatell error. Error " + errorNo + ", " + description, Integer.parseInt(errorNo));
                }
                JavaCIPUnknownScope.log_.debug("sendRequest: Got ID : " + ((String) objs[1]));
                idList.add(objs[1]);
            }
            responseReader.close();
        } catch (ParseRuntimeException ex) {
            throw new ClickatellRuntimeException("Unexpected response from Clickatell. : " + response, ClickatellRuntimeException.ERROR_UNKNOWN);
        }
