class c23125251 {
    ...
    public static HttpData postRequest(HttpPost postMethod, String xml) throws ClientProtocolRuntimeException, SocketRuntimeException, IORuntimeException, SocketTimeoutRuntimeException {
        ...
        try {
            ...
            StringEntity se = new StringEntity(xml, JavaCIPUnknownScope.HTTP.UTF_8);
            postMethod.setEntity(se);
            JavaCIPUnknownScope.printPostRequestHeader(postMethod);
            HttpParams httpParams = new BasicHttpParams();
            HttpConnectionParams.setConnectionTimeout(httpParams, JavaCIPUnknownScope.HTTP_TIMEOUT);
            HttpClient client = new DefaultHttpClient(httpParams);
            client.getParams().setIntParameter(HttpConnectionParams.CONNECTION_TIMEOUT, JavaCIPUnknownScope.DEFAULT_POST_REQUEST_TIMEOUT);
            client.getParams().setIntParameter(HttpConnectionParams.SO_TIMEOUT, JavaCIPUnknownScope.DEFAULT_POST_REQUEST_TIMEOUT);
            HttpResponse httpResponse = client.execute(postMethod);
            if (httpResponse == null)
                throw new SocketTimeoutRuntimeException();
            if (httpResponse.getStatusLine().getStatusCode() == 200) {
                byte[] bytearray = JavaCIPUnknownScope.ImageInputStream(httpResponse.getEntity());
