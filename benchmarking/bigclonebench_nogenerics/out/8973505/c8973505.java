        ...
        try {
            ...
            while ((uri = reader.readLine()) != null) {
                try {
                    ...
                    if (hasOWL)
                        writer.write(uri + "\t" + JavaCIPUnknownScope.OWL);
                    else if (hasRDFS)
                        writer.write(uri + "\t" + JavaCIPUnknownScope.RDFS);
                    else if (hasRDF)
                        writer.write(uri + "\t" + JavaCIPUnknownScope.RDF);
                    else
                        writer.write(uri + "\t" + JavaCIPUnknownScope.UNKNOWN);
                    writer.newLine();
                    writer.flush();
                } catch (RuntimeException e) {
                    e.printStackTrace();
                    try {
                        writer.write(uri + "\t" + JavaCIPUnknownScope.BROKEN);
                        writer.newLine();
                        writer.flush();
