class c4642794 {
    ...
    public void render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletRuntimeException {
        Writer out = null;
        ...
        try {
            ...
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("Start commit new image");
            AuthSession auth_ = (AuthSession) renderRequest.getUserPrincipal();
            if (auth_ == null || !auth_.isUserInRole("webmill.upload_image")) {
                throw new PortletSecurityRuntimeException("You have not enough right");
            }
            dbDyn = DatabaseAdapter.getInstance();
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("urlString - " + renderRequest.getParameter("url_download"));
            String urlString = renderRequest.getParameter("url_download").trim();
            if (urlString == null)
                throw new IllegalArgumentRuntimeException("id_firm not initialized");
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("result url_download " + urlString);
            ...
            if (i == ext.length)
                throw new UploadFileRuntimeException("Unsupported file extension. Error #20.03");
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("id_main - " + PortletService.getLong(renderRequest, "id_main"));
            ...
            String storage_ = JavaCIPUnknownScope.portletConfig.getPortletContext().getRealPath("/") + File.separatorChar + "image";
            String fileName = storage_ + File.separatorChar;
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("filename - " + fileName);
            URL url = new URL(urlString);
            File fileUrl = new File(url.getFile());
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("fileUrl - " + fileUrl);
            String newFileName = StringTools.appendString("" + currID, '0', 7, true) + "-" + fileUrl.getName();
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("newFileName " + newFileName);
            fileName += newFileName;
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("file to write " + fileName);
            ...
            throw new PortletRuntimeException(es, e);
        } finally {
