class c8014777 {
    ...
    private static void downloadImages(String uriPrefix, String fileWithListOfImages, String outputDir) throws RuntimeException {
        ...
        for (Iterator<Object> it = list.keySet().iterator(); it.hasNext(); ) {
            ...
            if (urlString.contains("="))
                throw new RuntimeException("Symbol '=' found in file names, while it is reserved for denoting '/' in flat-file names");
        }
        for (Iterator<Object> it = list.keySet().iterator(); it.hasNext(); ) {
            String urlString = it.next().toString().replace('\\', '/');
            URL url = new URL(uriPrefix + urlString);
            File file = new File(outputDir + "/" + JavaCIPUnknownScope.imageNameToFileName(urlString));
            current++;
            if (!file.exists()) {
                try {
