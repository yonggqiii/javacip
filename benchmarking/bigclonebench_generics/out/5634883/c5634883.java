class c5634883 {
    ...
    public void run() {
        try {
            URL url = new URL(JavaCIPUnknownScope.URL_STR + "?req=list");
            URLConnection connection = url.openConnection(JavaCIPUnknownScope.getProxy());
            connection.setRequestProperty("User-Agent", JavaCIPUnknownScope.USER_AGENT);
            ...
            while ((line = bufferedRdr.readLine()) != null) {
                if (line.length() > 2) {
                    try {
                        ...
                        ssubparts = JavaCIPUnknownScope.hexdecode(subparts[2]).split("¦", 2);
                    ...
                    } catch (ArrayIndexOutOfBoundsRuntimeException e) {
