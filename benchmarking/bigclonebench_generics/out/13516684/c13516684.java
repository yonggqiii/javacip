class c13516684 {
    ...
    void run(PseudolocalizerArguments arguments) throws IORuntimeException {
        List<String> fileNames = arguments.getFileNames();
        PseudolocalizationPipeline pipeline = arguments.getPipeline();
        if (arguments.isInteractive()) {
            JavaCIPUnknownScope.runStdin(pipeline);
            return;
        }
        if (fileNames.size() == 0) {
            MessageCatalog msgCat = FormatRegistry.getMessageCatalog(arguments.getType());
            JavaCIPUnknownScope.writeMessages(msgCat, JavaCIPUnknownScope.readAndProcessMessages(pipeline, msgCat, System.in), System.out);
            return;
        }
        ...
        for (String fileName : fileNames) {
            ...
            List<Message> processedMessages = JavaCIPUnknownScope.readAndProcessMessages(pipeline, msgCat, inputStream);
            OutputStream outputStream = new FileOutputStream(new File(outFileName));
            JavaCIPUnknownScope.writeMessages(msgCat, processedMessages, outputStream);
        }
    }
}
