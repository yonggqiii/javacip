    ...
    private void pushResource(String peerId, String communityId, String resourceFilePath, List<String> attachmentFilePaths) throws IOException {
        ...
        try {
            ...
            if (!resourceFile.exists()) {
                JavaCIPUnknownScope.LOG.error("JTellaAdapter: Resource file could not be found for push: " + resourceFilePath);
                return;
            }
            ...
            for (String attachmentPath : attachmentFilePaths) {
                ...
                if (!attachFile.exists()) {
                    JavaCIPUnknownScope.LOG.error("JTellaAdapter: Attachment file could not be found for push: " + attachmentPath);
                    return;
                }
                attachments.add(attachFile);
            }
            JavaCIPUnknownScope.LOG.debug("JTellaAdapter: Initiating push to: " + urlString);
            ...
            while (!fileWriteComplete) {
                ...
                JavaCIPUnknownScope.LOG.debug("JTellaAdapter: PUSHing file: " + nextFile.getAbsolutePath());
            ...
            inStream.close();
            JavaCIPUnknownScope.LOG.debug("JTellaAdapter: Push upload was succesful.");
        } catch (MalformedURLException ex) {
            JavaCIPUnknownScope.LOG.error("JTellaAdapter: pushResource Malformed URL: " + ex);
            throw new IOException("pushResource failed for URL: " + urlString);
        } catch (IOException ioe) {
            JavaCIPUnknownScope.LOG.error("JTellaAdapter: pushResource IOException: " + ioe);
            throw new IOException("pushResource failed for URL: " + urlString);
        } finally {
            try {
                if (fileInput != null) {
                ...
                JavaCIPUnknownScope.LOG.error("JTellaAdapter: pushResource failed to close connection streams.");
            }
        }
    }
}
