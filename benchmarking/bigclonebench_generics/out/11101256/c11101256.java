class c11101256 {
    ...
    private void innerJob(String inFrom, String inTo, String line, Map<String, Match> result) throws UnsupportedEncodingRuntimeException, IORuntimeException {
        ...
        while ((inLine = reader.readLine()) != null) {
            ...
            if (inLine.indexOf(homeTo) != -1) {
                result.get(JavaCIPUnknownScope.key).setHomeScore(inLine.substring(inLine.indexOf(scoreFrom) + scoreFrom.length(), inLine.indexOf(homeTo)));
                homeStart = true;
            }
            if (homeStart && inLine.indexOf(goalInclud) != -1) {
                MatchEvent me = new MatchEvent();
                me.setPlayerName(JavaCIPUnknownScope.getPlayerName(inLine.substring(inLine.indexOf(playerFrom) + playerFrom.length(), inLine.indexOf(playerTo))));
                me.setTime(inLine.substring(inLine.indexOf(timeFrom) + timeFrom.length(), inLine.indexOf(timeTo)));
                result.get(JavaCIPUnknownScope.key).getHomeEvents().add(me);
            }
            if (inLine.indexOf(awayTo) != -1) {
                result.get(JavaCIPUnknownScope.key).setAwayScore(inLine.substring(inLine.indexOf(scoreFrom) + scoreFrom.length(), inLine.indexOf(awayTo)));
                awayStart = true;
            }
            if (awayStart && inLine.indexOf(goalInclud) != -1) {
                ...
                me.setPlayerName(JavaCIPUnknownScope.getPlayerName(inLine.substring(inLine.indexOf(playerFrom) + playerFrom.length(), inLine.indexOf(playerTo))));
                me.setTime(inLine.substring(inLine.indexOf(timeFrom) + timeFrom.length(), inLine.indexOf(timeTo)));
                result.get(JavaCIPUnknownScope.key).getAwayEvents().add(me);
            }
        }
        reader.close();
    }
