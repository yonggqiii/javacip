class c1327097 {
    ...
    public String generateMappackMD5(File mapPackFile) throws IORuntimeException, NoSuchAlgorithmRuntimeException {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                JavaCIPUnknownScope.log.trace("Hashsum " + Hex.encodeHexString(digest) + " includes \"" + name + "\"");
                md5Total.update(digest);
                md5Total.update(name.getBytes());
            }
            String md5sum = Hex.encodeHexString(md5Total.digest());
            JavaCIPUnknownScope.log.trace("md5sum of " + mapPackFile.getName() + ": " + md5sum);
            return md5sum;
        } finally {
            zip.close();
        }
