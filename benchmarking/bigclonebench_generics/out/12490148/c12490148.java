class c12490148 {
    ...
    private void createIDocPluginProject(IProgressMonitor monitor, String sourceFileName, String pluginName, String pluginNameJCo) throws CoreRuntimeException, IORuntimeException {
        monitor.subTask(MessageFormat.format(Messages.ProjectGenerator_CreatePluginTaskDescription, pluginName));
        final Map<String, byte[]> files = JavaCIPUnknownScope.readArchiveFile(sourceFileName);
        monitor.worked(10);
        IProject project = JavaCIPUnknownScope.workspaceRoot.getProject(pluginName);
        if (project.exists()) {
        ...
        description.setNatureIds(new String[] { JavaCore.NATURE_ID, JavaCIPUnknownScope.PLUGIN_NATURE_ID });
        ...
        JavaCIPUnknownScope.writeTextFile(monitor, manifest, metaInfFolder.getFile("MANIFEST.MF"));
        ...
        JavaCIPUnknownScope.writeTextFile(monitor, buildProperties, project.getFile("build.properties"));
        JavaCIPUnknownScope.exportableBundles.add(JavaCIPUnknownScope.modelManager.findModel(project));
    }
}
