class c7781593 {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionRuntimeException {
        ...
        DirectoryDialog fd = new DirectoryDialog(activeShell, JavaCIPUnknownScope.SWT.SAVE);
        ...
        if (open != null) {
            ...
            CancelableRunnable runnable = new CancelableRunnable() {
                ...
                protected IStatus runCancelableRunnable(IProgressMonitor monitor) {
                    ...
                    monitor.beginTask(JavaCIPUnknownScope.NLS.bind(Messages.SaveFileOnDiskHandler_SavingFiles, open), informationUnitsFromExecutionEvent.size());
                    for (InformationUnit informationUnit : informationUnitsFromExecutionEvent) {
                        if (!monitor.isCanceled()) {
                            monitor.setTaskName(JavaCIPUnknownScope.NLS.bind(Messages.SaveFileOnDiskHandler_Saving, informationUnit.getLabel()));
                            ...
                            try {
                                ...
                                returnValue = StatusCreator.newStatus(JavaCIPUnknownScope.NLS.bind(Messages.SaveFileOnDiskHandler_ErrorSaving, informationUnit.getLabel(), e));
                                break;
                            } finally {
                                if (writer != null) {
                                    try {
