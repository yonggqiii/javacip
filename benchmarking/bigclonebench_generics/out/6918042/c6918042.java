class c6918042 {
    ...
    public void updateMetaList() {
        JavaCIPUnknownScope.metaserverModel.begin();
        JavaCIPUnknownScope.serverCache.expire(JavaCIPUnknownScope.EXPIRE_INTERVAL * 1000);
        final Map<String, MetaserverEntry> oldEntries = JavaCIPUnknownScope.serverCache.getAll();
        ...
        JavaCIPUnknownScope.metaserverModel.add(localhostMetaserverEntry);
        oldEntries.remove(ServerCache.makeKey(localhostMetaserverEntry));
        JavaCIPUnknownScope.serverCache.put(localhostMetaserverEntry);
        try {
            final URL url = new URL(JavaCIPUnknownScope.METASERVER_URL);
                        ...
                        try {
                            ...
                            while (true) {
                                ...
                                if (metaserverEntry != null) {
                                    JavaCIPUnknownScope.metaserverModel.add(metaserverEntry);
                                    oldEntries.remove(ServerCache.makeKey(metaserverEntry));
                                    JavaCIPUnknownScope.serverCache.put(metaserverEntry);
                                }
                            }
                        } finally {
                            br.close();
        ...
        JavaCIPUnknownScope.metaserverModel.commit();
        JavaCIPUnknownScope.serverCache.save();
    }
}
