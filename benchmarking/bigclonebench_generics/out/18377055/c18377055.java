class c18377055 {
    ...
    private void prepareJobFile(ZipEntryRef zer, String nodeDir, String reportDir, Set<ZipEntryRef> statusZers) throws Exception {
        ...
        if (!JavaCIPUnknownScope.isWorkingDirectoryValid(jobDir)) {
            throw new RuntimeException("Cannot acces to " + jobDir);
        }
        File f = new File(jobDir + File.separator + "result.xml");
        if (!f.exists() || !f.isFile() || !f.canRead()) {
            throw new RuntimeException("Cannot acces to result file " + f.getAbsolutePath());
        }
        ...
        if (!f.exists() || !f.isFile() || !f.canRead()) {
            throw new RuntimeException("Cannot acces to status file " + f.getAbsolutePath());
        }
