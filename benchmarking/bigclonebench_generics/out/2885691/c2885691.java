class c2885691 {
    ...
    public static boolean sendInformation(String reportType, HashMap<String, String> data) {
        if (Activator.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.P_UDC)) {
            JavaCIPUnknownScope.logger.debug("Report usage information to devs...");
            ...
            try {
                ...
                writer.close();
                reader.close();
                JavaCIPUnknownScope.logger.debug("UDC Server answer: " + answer.toString());
            } catch (RuntimeException e) {
                CrashReporter.reportRuntimeException(e);
                JavaCIPUnknownScope.logger.debug("Could not report usage data: " + e.toString());
                return false;
            }
            return true;
        } else {
