class c9918348 {
    ...
    public static ArrayList[] imageSearch(String imageQuery, int startingIndex) {
        ...
        } catch (UnsupportedEncodingRuntimeException e1) {
            e1.printStackTrace();
        }
        ...
        Matcher m = JavaCIPUnknownScope.imgBlock.matcher(result);
        while (m.find()) {
            String s = m.group();
            Matcher imgM = JavaCIPUnknownScope.imgurl.matcher(s);
            imgM.find();
            String url = imgM.group(1);
            Matcher srcM = JavaCIPUnknownScope.imgsrc.matcher(s);
            srcM.find();
