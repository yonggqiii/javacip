class c9036637 {
    ...
    public static boolean reportRuntimeException(Throwable ex, HashMap<String, String> suppl) {
        if (Activator.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.P_CRASH_REPORTING)) {
            JavaCIPUnknownScope.logger.debug("Report exception to devs...");
            ...
            try {
                ...
                writer.close();
                reader.close();
                JavaCIPUnknownScope.logger.debug(answer.toString());
            } catch (RuntimeException e) {
                JavaCIPUnknownScope.logger.debug("Could not report exception");
                return false;
            }
            return true;
        } else {
            JavaCIPUnknownScope.logger.debug("Reporting not wished!");
            return false;
        }
    }
}
