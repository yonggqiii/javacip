class c19919874 {
    ...
    public Long createSite(Site site, List<String> hosts) {
        if (JavaCIPUnknownScope.log.isDebugEnabled()) {
            JavaCIPUnknownScope.log.debug("site: " + site);
            if (site != null) {
                JavaCIPUnknownScope.log.debug("    language: " + site.getDefLanguage());
                JavaCIPUnknownScope.log.debug("    country: " + site.getDefCountry());
                JavaCIPUnknownScope.log.debug("    variant: " + site.getDefVariant());
                JavaCIPUnknownScope.log.debug("    companyId: " + site.getCompanyId());
            }
        }
        ...
        try {
            ...
            if (JavaCIPUnknownScope.log.isDebugEnabled())
                JavaCIPUnknownScope.log.debug("Count of inserted records - " + i1);
            if (hosts != null) {
                for (String s : hosts) {
