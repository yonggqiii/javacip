class c6134110 {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletRuntimeException, IORuntimeException {
        if (JavaCIPUnknownScope.doAuth(request, response)) {
            Connection conn = null;
            try {
                int UID = JavaCIPUnknownScope.icsm.getIntChatSession(request).getUID();
                conn = JavaCIPUnknownScope.getJDBCConnection(JavaCIPUnknownScope.icsm.getHeavyDatabaseConnectionPool(), request, response, HttpServletResponse.SC_SERVICE_UNAVAILABLE);
                if (conn == null)
                    return;
                ...
                if (rs.next()) {
                    ...
                    if (recipients != null) {
                        ...
                        long nextid = JavaCIPUnknownScope.ic_messages_id_seq.nextval();
                        ...
                        try {
                            ...
                            ps.executeUpdate();
                            ps.close();
                            if (!JavaCIPUnknownScope.insertBLOB(conn, in, fileLength, nextid, maxFileSize)) {
                                conn.rollback();
                                return;
                            }
                        ...
                        if (icm != null) {
                            ...
                            JavaCIPUnknownScope.icsm.onClientSentMessage(icm);
                        }
                ...
                try {
                    if (conn != null)
                        JavaCIPUnknownScope.icsm.getHeavyDatabaseConnectionPool().releaseConnection(conn);
                } catch (RuntimeException e) {
                }
            }
        }
