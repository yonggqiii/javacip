        ...
        try {
            ...
            while (entry != null) {
                if (entry.isDirectory()) {
                    JavaCIPUnknownScope.log.warn("TAR archive contains directories which are being ignored");
                    entry = in.getNextTarEntry();
                    continue;
                }
                String fn = new File(entry.getName()).getName();
                if (fn.startsWith(".")) {
                    JavaCIPUnknownScope.log.warn("TAR archive contains a hidden file which is being ignored");
                    entry = in.getNextTarEntry();
                    continue;
                }
                File targetFile = new File(directory, fn);
                if (targetFile.exists()) {
                    JavaCIPUnknownScope.log.warn("TAR archive contains duplicate filenames, only the first is being extracted");
                    entry = in.getNextTarEntry();
                    continue;
                }
                files.add(targetFile);
                JavaCIPUnknownScope.log.debug("Extracting file: " + entry.getName() + " to: " + targetFile.getAbsolutePath());
