class c8956542 {
    ...
    public static InputStream retrievePricesHTML(String username, String password) throws IOException, SAXException {
        ...
        JavaCIPUnknownScope.setCookies(cookies, loginConnection);
        ...
        postCredsConnection.setRequestProperty("Cookie", JavaCIPUnknownScope.buildCookieString(cookies));
        OutputStreamWriter postCredsWriter = new OutputStreamWriter(postCredsConnection.getOutputStream());
        postCredsWriter.append("__VIEWSTATE=");
        postCredsWriter.append(URLEncoder.encode(viewStateValue, "UTF-8"));
        postCredsWriter.append('&');
        postCredsWriter.append("Login_Module1%3Ausername=");
        postCredsWriter.append(URLEncoder.encode(username, "UTF-8"));
        postCredsWriter.append('&');
        postCredsWriter.append("Login_Module1%3Apassword=");
        postCredsWriter.append(URLEncoder.encode(password, "UTF-8"));
        postCredsWriter.append('&');
        postCredsWriter.append("Login_Module1%3AButtonLogin.x=0");
        postCredsWriter.append('&');
        ...
        if (postResponseCode == 302) {
            JavaCIPUnknownScope.setCookies(cookies, postCredsConnection);
            ...
            dataConnection.setRequestProperty("Cookie", JavaCIPUnknownScope.buildCookieString(cookies));
