class c7149578 {
    ...
    protected void parseContent(StreamLimiter streamLimiter, LanguageEnum forcedLang) throws IOException {
        ...
        if (sourceDocument != null && JavaCIPUnknownScope.urlItemFieldEnum != null) {
            String name = JavaCIPUnknownScope.urlItemFieldEnum.contentTypeCharset.getName();
            FieldValueItem fieldValueItem = sourceDocument.getFieldValue(name, 0);
            if (fieldValueItem != null)
                charset = fieldValueItem.getValue();
            if (charset == null) {
                name = JavaCIPUnknownScope.urlItemFieldEnum.contentEncoding.getName();
        ...
        for (HtmlNodeAbstract<?> node : htmlProvider.getMetas()) {
            ...
            if ("keywords".equalsIgnoreCase(attr_name))
                JavaCIPUnknownScope.addField(ParserFieldEnum.meta_keywords, metaContent);
            else if ("description".equalsIgnoreCase(attr_name))
                JavaCIPUnknownScope.addField(ParserFieldEnum.meta_description, metaContent);
        ...
        if (!metaRobotsNoIndex) {
            ...
            if (nodes != null && nodes.size() > 0) {
                ...
                JavaCIPUnknownScope.getBodyTextContent(sb, x, true, null);
        ...
        if (lang != null) {
            ...
            JavaCIPUnknownScope.addField(ParserFieldEnum.lang, ll);
