class c19467540 {
    ...
    public boolean updateProductIfAvailable(Map<String, Integer> carro, HttpServletRequest request, Map<Producto, Integer> listado) {
        ...
        boolean exito = false;
        try {
            conexion = JavaCIPUnknownScope.pool.getConnection();
            conexion.setAutoCommit(false);
            select = conexion.prepareStatement("SELECT* FROM " + JavaCIPUnknownScope.nameBD + ".Productos WHERE Codigo=?");
            update = conexion.prepareStatement("UPDATE " + JavaCIPUnknownScope.nameBD + ".Productos SET Stock=? WHERE Codigo=?");
            String codigoProd;
            ...
            JavaCIPUnknownScope.logger.log(Level.SEVERE, "Error actualizando unidades de productos en compra", ex);
            try {
                conexion.rollback();
            } catch (SQLException ex1) {
                JavaCIPUnknownScope.logger.log(Level.SEVERE, "Error haciendo rolback de la transacción que ha dado error en la actualización de unidades por compra", ex1);
            }
        } finally {
            JavaCIPUnknownScope.cerrarConexionYStatement(conexion, select, update);
            JavaCIPUnknownScope.cerrarResultSet(rs);
        }
        return exito;
    }
}
