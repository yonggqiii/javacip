class c8126683 {
    ...
    public Resource readResource(URL url, ResourceManager resourceManager) throws NAFRuntimeException {
        ...
        try {
            ...
            Document doc = JavaCIPUnknownScope.docbuilder.parse(in);
            for (Node n = doc.getFirstChild(); n != null; n = n.getNextSibling()) {
                if (n.getNodeType() == Node.PROCESSING_INSTRUCTION_NODE && "xml-stylesheet".equals(n.getNodeName())) {
                    ...
                    if ("text/xsl".equals(attrs.get("type"))) {
                        ...
                        if (href == null)
                            throw new NAFRuntimeException("Style sheet processing instructions must have an \"href\" attribute");
                        try {
                            Transformer t = JavaCIPUnknownScope.styleManager.createTransformer(new URL(url, href));
                            if (trList == null)
            ...
            if (trList != null) {
                for (Transformer t : trList) {
                    doc = (Document) JavaCIPUnknownScope.styleManager.transform(t, doc);
                    if (JavaCIPUnknownScope.LOGGER_DUMP.isDebugEnabled()) {
                        StringWriter swr = new StringWriter();
                        DOMUtil.dumpNode(doc, swr);
                        JavaCIPUnknownScope.LOGGER_DUMP.debug("Transformed instance:\n" + swr + "\n");
                    }
                }
            }
            Element rootE = doc.getDocumentElement();
            if (!JavaCIPUnknownScope.NAF_NAMESPACE_URI.equals(rootE.getNamespaceURI()))
                throw new NAFRuntimeException("Root element does not use the NAF namespace");
            Object comp = JavaCIPUnknownScope.createComponent(rootE, resource, null);
