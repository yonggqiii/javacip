class c17404914 {
    ...
    public static boolean installMetricsCfg(Db db, String xmlFileName) throws RuntimeException {
        String xmlText = FileHelper.asString(xmlFileName);
        Bundle bundle = new Bundle();
        JavaCIPUnknownScope.loadMetricsCfg(bundle, xmlFileName, xmlText);
        try {
            db.begin();
            ...
            if (!xmlFileName.equals(fi.filename))
                throw new IllegalStateRuntimeException("FileInformation bad for " + xmlFileName);
            ...
            if (1 != db.executeUpdate(psAdd)) {
                throw new IllegalStateRuntimeException("Could not add " + xmlFileName);
            }
            ...
            while (i.hasNext()) {
                ...
                if (1 != db.executeUpdate(psAddGroup))
                    throw new IllegalStateRuntimeException("Could not add group " + grp.groupName + " from " + xmlFileName);
                ...
                while (j.hasNext()) {
                    ...
                    if (1 != db.executeUpdate(psAddMetric))
                        throw new IllegalStateRuntimeException("Could not add " + metricName + " from " + xmlFileName);
                    int metricId = DbHelper.getIntKey(psMetricId);
                    psAddGroup2Metric.setInt(1, metricId);
                    if (1 != db.executeUpdate(psAddGroup2Metric))
                        throw new IllegalStateRuntimeException("Could not add group " + grp.groupName + " -> " + metricName + " from " + xmlFileName);
                }
            }
            return true;
