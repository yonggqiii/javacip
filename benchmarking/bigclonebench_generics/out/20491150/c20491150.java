            ...
            if (directory.exists()) {
                ...
                for (String file : files) {
                    if (file.endsWith(".class")) {
                        JavaCIPUnknownScope.addInstanceIfCommand(JavaCIPUnknownScope.pckgname + '.' + file.substring(0, file.length() - 6), cmds);
                    }
                }
            } else {
                ...
                while (entriesEnum.hasMoreElements()) {
                    ...
                    if (entryname.startsWith(starts) && (entryname.lastIndexOf('/') <= starts.length()) && entryname.endsWith(".class")) {
                        ...
                        JavaCIPUnknownScope.addInstanceIfCommand(classname, cmds);
                    }
                }
            }
        } catch (IORuntimeException ioe) {
            JavaCIPUnknownScope.LOG.warning("couldn't retrieve classes of " + url + ". Reason: " + ioe);
        }
    }
}
