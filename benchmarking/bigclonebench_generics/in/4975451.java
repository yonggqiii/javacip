    ...
    public void performUpdates(List<PackageDescriptor> downloadList, ProgressListener progressListener) throws IOException, UpdateServiceException_Exception {
        ...
        try {
            for (PackageDescriptor desc : downloadList) {
                ...
                String urlString = service.getDownloadURL(pid, version, platformName);
                ...
                if (typeName.equals("RAPIDMINER_PLUGIN")) {
                    ...
                    if (incremental) withBaseVersion = "?baseVersion=" + baseVersionURIEncoded;
                    URI uri1 = UpdateManager.getUpdateServerURI(urlString + withBaseVersion);
                    URL url = uri1.toURL();
                    if (incremental) {
                        root.info("Updating " + pid + " incrementally.");
                        try {
                            Stream s = openStream(url, progressListener, minProgress, maxProgress);
                            updatePluginIncrementally(extension, s, baseVersion, version);
                        } catch (IOException e) {
                    ...
                    if (!incremental) {
                        ...
                        Stream s = openStream(url, progressListener, minProgress, maxProgress);
                        updatePlugin(extension, s, version);
                    }
