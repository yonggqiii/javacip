...
os.chdir('tests')
os.chdir('bigclonebench_generics/in')
all_files = os.listdir()
os.chdir('..')

with open('generic_files.txt', 'r+') as f:
...
files = untouched_files + failed_files
...
while files:
    ...
    for file in batch:
        ...
        if not os.path.exists(outdir):
            ...
            start_time = time()
            p = subprocess.Popen(
                ['java', '-jar', '../../target/scala-3.0.2/javacip-0.0.1.jar', '-h', '-n', '-o', f'{outdir}', '-i', '300000', infile])
            try:
                p.wait(TIMEOUT)
